...
import { X, Upload, FileText, Sparkles, Loader2, CheckCircle2, AlertCircle, Info } from 'lucide-react';
...
export default function SmartImporter({ onClose, onImport, categories }: SmartImporterProps) {
            ...
            <>
              ...
              >
                ...
                <p className="text-xs font-bold text-brand-gray-muted uppercase tracking-widest text-center">
                  {isDragActive ? "Drop to Scan" : "Or drop CSV/Text file"}
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  {preview.expenses.map((exp, idx) => (
                    <div key={`exp-${idx}`} className="p-5 bg-brand-gray-light/30 rounded-3xl border border-transparent hover:border-brand-gray-light transition-all flex items-center justify-between">
                      <div>
                        <p className="text-xs font-bold text-brand-black">{exp.description}</p>
                        <span className="text-[9px] font-black uppercase tracking-[0.1em] text-brand-gray-muted px-2 py-0.5 bg-white rounded-lg inline-block mt-1">{exp.category}</span>
                      </div>
                      <span className="text-sm font-display font-bold text-red-500">-₹{exp.amount}</span>
                    </div>
                  ))}
                  {preview.income.map((inc, idx) => (
                    <div key={`inc-${idx}`} className="p-5 bg-brand-accent/5 rounded-3xl border border-brand-accent/10 flex items-center justify-between">
                      <div>
                        <p className="text-xs font-bold text-brand-black">{inc.description}</p>
                        <span className="text-[9px] font-black uppercase tracking-[0.1em] text-brand-accent px-2 py-0.5 bg-white rounded-lg inline-block mt-1">Income</span>
                      </div>
                      <span className="text-sm font-display font-bold text-brand-accent">+₹{inc.amount}</span>
                    </div>
                  ))}
                </div>
              </div>
